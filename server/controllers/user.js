...
function signUp(req, res) {
  ...
  if (!password || !repeatPassword) {
    res
      .status(404)
      .send({ message: "Las contraseñas son obligatorias", ok: false });
  } else {
    if (password != repeatPassword) {
      res
        .status(404)
        .send({ message: "Las contraseñas no son iguales", ok: false });
    } else {
      bcrypt.hash(password, null, null, function (error, hash) {
        if (error) {
          res
            .status(500)
            .send({ message: "Error al encriptar la contraseña", ok: false });
        } else {
          user.password = hash;
          user.save(function (error, userStored) {
            if (error) {
              res
                .status(500)
                .send({ message: "Error al crear usuario", ok: false });
            } else {
              if (!userStored) {
                res
                  .status(404)
                ...
                res.status(200).send({
                  message: "Usuario creaado correctamente",
                  user: userStored,
                  ok: true,
                });
              }
